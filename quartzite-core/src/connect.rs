...
mod tests {
    ...
    // Zero-arg signal object used by arity tests as a source or recording target.
    struct NullRecv {
        base: ObjectBase,
        pub sig: Signal<()>,
    }
    ...
    // Recording variant of NullRecv: sets an AtomicBool flag when `emit_signal("sig", _)` is
    // called. Used to verify the forwarding callback fires after truncation.
    struct RecordingNullRecv {
        base: ObjectBase,
        pub sig: Signal<()>,
        pub fired: Arc<std::sync::atomic::AtomicBool>,
    }
    impl crate::AsObject for RecordingNullRecv {
        fn object_base(&self) -> &ObjectBase {
            &self.base
        }
        fn object_base_mut(&mut self) -> &mut ObjectBase {
    ...
    impl crate::Object for RecordingNullRecv {
        fn meta_object(&self) -> &'static MetaObject {
            // Shares NullRecv's meta: one 0-arg signal named "sig".
            &NULL_META
        }
        fn read_property(&self, _: &str) -> Option<Value> {
            None
    ...
    fn arity_mismatch_returns_error() {
        ...
        // Use a 0-arg source (NullRecv) → 1-arg target (Receiver) to keep the rejection path
        // exercised: ArityMismatch { from: 0, to: 1 }.
        ...
        let to = Arc::new(Mutex::new(Receiver::new())) as Arc<Mutex<dyn Object>>;
        let err =
            connect_signal_to_signal(&mut source, "sig", &to, "sig_b", ConnectionType::Direct)
                .unwrap_err();
    ...
    // Two-arg sender used by typed-path arity and truncation tests.
    struct Sender2 {
        base: ObjectBase,
            ...
            match signal {
                "sig" => {
                    ...
                    Some(self.sig.connect_typed(
                        move |args: &(i32, i32)| {
                            (*cb)(&[
                                crate::IntoValue::into_value(args.0),
                                crate::IntoValue::into_value(args.1),
                            ]);
                        },
                        conn_type,
                    ))
        ...
        fn emit_signal(&mut self, signal: &str, args: &[Value]) -> Option<()> {
            match signal {
                "sig" => {
                    ...
                    let a: i32 = crate::FromValue::from_value(args[0].clone()).ok()?;
                    let b: i32 = crate::FromValue::from_value(args[1].clone()).ok()?;
                    crate::emit!(self.sig, &(a, b));
                    Some(())
                }
                _ => None,
            }
    ...
    // Two-arg receiver used by the flipped connect_signals_typed arity-mismatch test.
    struct BigReceiver {
        base: ObjectBase,
        pub _sig: Signal<(i32, i32)>,
    }
    static BIG_RECV_PARAMS: [crate::meta::ParamMeta; 2] = [
        crate::meta::ParamMeta::new("a", "i32"),
        crate::meta::ParamMeta::new("b", "i32"),
    ];
    static BIG_RECV_SIGS: [crate::meta::SignalMeta; 1] =
        [crate::meta::SignalMeta::new("sig", &BIG_RECV_PARAMS)];
    ...
    impl crate::AsObject for BigReceiver {
        fn object_base(&self) -> &ObjectBase {
            &self.base
        }
        fn object_base_mut(&mut self) -> &mut ObjectBase {
    ...
    impl crate::Object for BigReceiver {
        fn meta_object(&self) -> &'static MetaObject {
            &BIG_RECV_META
        }
        fn read_property(&self, _: &str) -> Option<Value> {
    ...
    // the same checks inside the typed connect_signals wrapper.)

    #[test]
    fn connect_signals_typed_arity_mismatch_returns_error() {
        // New rule: from_arity < to_arity is the only rejection.
        // Use Sender (1-arg sig_a) → BigReceiver (2-arg sig) to keep rejection exercised:
        // ArityMismatch { from: 1, to: 2 }.
        let mut sender = Sender::new();
    ...
    // Hand-rolled Object whose invoke_method records the received args slice.
    // Used by connect_signal_to_slot meta-validated-path tests (subtask 6).
    ...
    // Three static MetaObject instances with differing lookup_method behaviour.
    // (a) Validated 1-arg i32 slot — lookup_method returns Some for "on_click".
    static RSR_PARAMS_1ARG: [crate::meta::ParamMeta; 1] = [crate::meta::ParamMeta::new("v", "i32")];
    static RSR_METHODS_1ARG: [crate::meta::MethodMeta; 1] = [crate::meta::MethodMeta::new(
        "on_click",
        &RSR_PARAMS_1ARG,
        "()",
    )];
    ...
    // (b) Validated 1-arg bool slot.
    static RSR_PARAMS_BOOL: [crate::meta::ParamMeta; 1] =
        [crate::meta::ParamMeta::new("v", "bool")];
    static RSR_METHODS_BOOL: [crate::meta::MethodMeta; 1] = [crate::meta::MethodMeta::new(
        "on_click",
        &RSR_PARAMS_BOOL,
        "()",
    )];
    ...
    // (c) Fallback — noop_lookup_method; slot not in meta-system.
    ...
    impl crate::AsObject for RecordingSlotRecv {
        fn object_base(&self) -> &ObjectBase {
            &self.base
        }
        fn object_base_mut(&mut self) -> &mut ObjectBase {
    ...
    impl crate::Object for RecordingSlotRecv {
        fn meta_object(&self) -> &'static MetaObject {
            self.meta
        }
        fn read_property(&self, _: &str) -> Option<Value> {

...
- **State derivations:** fill/text precedence `disabled > pressed > checked > hovered > idle`. Hover and Pressed are derived per `c.blend(WindowText.Normal, t)` with `t = 0.06 / 0.16` (added by issue #402's `ColorGroup` axis). Disabled = α × 0.5. Focus = additive 2 px `ColorRole::FocusRing` outline (new role, defaults to `Highlight`).

    ...
    pub fn start(&mut self, driver: Arc<dyn TimerDriver>) {
        ...
        let callback: Box<dyn Fn() + Send + Sync + 'static> = Box::new(move || {
            ...
            if !state.signals_blocked.load(Ordering::Relaxed)
                && let Ok(mut sig) = state.signal.lock()
            {
                sig.emit(&(count,));
            }
            if single_shot {
...
///
/// Multiple [`Timer`]s can share one `Arc<PoolDriver>`. The pool dispatches ticks using a
/// `BinaryHeap` of `(Instant, ObjectId)` deadlines and a single background thread.
///
/// # Examples
///
/// ```no_run

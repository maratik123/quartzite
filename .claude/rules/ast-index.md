...
The `SessionStart` Hook in `.claude/settings.json` runs `ast-index update`
(or `rebuild` if no index exists) every time a session opens, and the
`PostToolUse` Hook (Matcher: `Bash`, trigger: `git commit`) refreshes the
index after each commit. **Manual `ast-index update` is rarely needed** —
only run it explicitly after a large external file change you want indexed
before the next search (e.g. right after `git pull` or a checkout,
mid-session).
...
The `Explore` and `general-purpose` Subagents are the primary search
Subagents in this project — give them the block above whenever they're
tasked with locating code.
...
Real Quartzite symbols — the Subagent makes better choices when it has
...
- Index is stale — run `ast-index update` and retry (rare; the
  `SessionStart` + `PostToolUse` `Bash` Hooks normally keep it fresh).

...
4. `cargo fmt`
5. `RUSTDOCFLAGS="-D warnings -D missing-docs" cargo doc --no-deps --workspace --features serde` — clean (`--features serde` required so intra-doc links into the `serde`-gated `snapshot` module resolve — matches CI)

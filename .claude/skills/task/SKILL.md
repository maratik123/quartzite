...
- After each subtask:
  ...
  5. **N=3 of M≥5 handoff gate (binding, not optional).** If the just-completed subtask is the **3rd** of a planned total **M≥5**, STOP coding in this turn and spawn a `/context-reset` handoff via `Agent`. See `reference.md` § N=3 of M≥5 handoff gate (rationale) for the failure mode this prevents and `.claude/skills/context-reset/SKILL.md` for the handoff protocol.

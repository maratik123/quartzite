...
- After each subtask:
  ...
  5. **N=3 of M≥5 handoff gate (binding, not optional).** If the just-completed subtask is the **3rd** of a planned total **M≥5**, STOP coding in this turn and spawn a `/context-reset` handoff via `Agent`. **The design's `## Handoff plan` section is the source of truth for the trigger boundary** — read it at the start of Step 8 and use it to anticipate when this gate will fire (per `.claude/agents/design.md` § Rules → handoff-grouping). The runtime per-subtask `N == 3` count is now a **cross-check** against the design's pre-computed plan: if the runtime counter says fire-now but the design's `## Handoff plan` doesn't mark a boundary here (or vice versa), trigger the **Design Amendment recipe** (`reference.md` § Design Amendment recipe) — do NOT silently skip the gate. See `reference.md` § N=3 of M≥5 handoff gate (rationale) for the failure mode this prevents and `.claude/skills/context-reset/SKILL.md` for the handoff protocol.
...
**Reference:** [`reference.md`](reference.md) — anti-patterns, gate checklist, Design Amendment recipe, validation procedures, FORBIDDEN list, Step-specific narrative detail (N=3 handoff rationale, local-FAIL investigation, panic-index sync, Step 11 review-fix narrative, Step 12 inbox-propagation rules).

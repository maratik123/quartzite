...
## N=3 of M≥5 handoff gate (rationale)

If the just-completed subtask is the **3rd** of a planned total **M≥5**, STOP further coding in this conversation turn. The next action MUST be a `/context-reset` handoff spawned via `Agent`. Skipping this gate has caused compaction mid-task to silently drop the `/task` step contract — Step 10 (self-review) was omitted on PR #339 because the compaction summary did not reproduce the strict sequence faithfully. Re-state the gate to yourself before deciding: *"I completed subtask N of M. N == 3? M >= 5? If both yes → handoff. No exceptions for 'almost done' or 'one more quick subtask'."* See `.claude/skills/context-reset/SKILL.md` for the handoff protocol.

**Trigger source: design's `## Handoff plan` section.** As of #371, the design agent produces a `## Handoff plan` section in the design document for every M ≥ 5 decomposition (per `.claude/agents/design.md` § Rules → handoff-grouping). That section names the exact subtask-after-which boundary each `/context-reset` handoff fires — pre-computed at design time rather than re-derived at execution time. The binding rule above ("if N == 3 AND M >= 5 then handoff") is unchanged; what changes is the *recording surface* of the boundary: read the design's `## Handoff plan` at the start of Step 8 and use it to anticipate which subtask numbers will trigger handoff. The runtime `N == 3` count remains a **cross-check** against the design's plan — if the two disagree (runtime says fire here but the plan has no boundary, or vice versa), the disagreement is a Design Amendment trigger (§ Design Amendment recipe above), not a silent skip. For M ≤ 4 designs no `## Handoff plan` is emitted (auto-PASS in design-review) and the gate never fires.

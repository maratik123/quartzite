...
- Generic simple fn missing the `_Simple._` doc tag (italic line `/// _Simple._` directly under the summary, before any `#` heading)? **"Generic" means: own `<T>` / `<F: ...>` / `impl Trait` parameter, OR method inside an `impl<T> ...` / `impl<T> Trait for Foo<T> { … }` block where `Self` is parametrised by the impl block's generics — even when the method itself declares no extra type parameters.** The carve-out keeping default trait methods on a `pub trait` body in the concrete row applies *only* to default methods inside the trait declaration, NOT to methods inside `impl<T> ...` blocks. Conversely, a concrete fn carrying `_Simple._` instead of `#[inline]` (or an `impl<T> Trait for Foo<T>` method carrying `#[inline]` instead of the tag) → `minor` (wrong marker for the shape). See `AGENTS.md` Code Style → `#[inline]` and the `_Simple._` doc tag.

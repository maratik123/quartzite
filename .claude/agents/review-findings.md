...
- **Trace significant state mutations (AGENTS.md "Code Style"):** fns that meaningfully mutate application state without an opening `debug!` call → `minor`. Sibling of an already-traced fn missing the trace → `major` (breaks log-story consistency). High-frequency paths (signal emit and similar) using always-on `debug!`/`trace!` rather than the `verbose-tracing` cargo feature → `major`.

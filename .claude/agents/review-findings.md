...
- Generic simple fn (or simple-bodied default trait method on a generic blanket impl) missing the `_Simple._` doc tag (italic line `/// _Simple._` directly under the summary, before any `#` heading)? Conversely, a concrete fn carrying `_Simple._` instead of `#[inline]` (or a generic fn carrying `#[inline]` instead of the tag) → `minor` (wrong marker for the shape). See `AGENTS.md` Code Style → `#[inline]` and the `_Simple._` doc tag.

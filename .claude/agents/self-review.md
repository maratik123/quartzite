...
- Every simple generic fn (and every simple-bodied default trait method on a generic blanket impl) added by this diff carries the `_Simple._` doc tag (italic line `/// _Simple._` directly under the summary, before any `#` heading)? Wrong marker for the shape (concrete with tag, or generic with `#[inline]`) → REJECT. See `AGENTS.md` Code Style → `#[inline]` and the `_Simple._` doc tag.

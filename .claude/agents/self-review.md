...
- **Trace significant state mutations (AGENTS.md "Code Style"):** any new fn that meaningfully mutates application state (tree mutations, lifecycle transitions, index updates, config changes) without an opening `debug!` call → `nit` with rationale. When the diff adds a sibling to an already-traced fn, missing trace on the new sibling → REJECT (consistency). High-frequency paths (signal emit and similar) must be gated behind the `verbose-tracing` cargo feature, not always-on.

...
**Superseded by:** PR #364 — `.claude/agents/self-improve.md` § Step 6 now carries a *Primitive-absence statement* + *pause-and-surface protocol*; the silent-substitution failure mode this entry records is contractually prevented (Step 6 handoff yields to the parent thread instead of attempting same-context close-reads).

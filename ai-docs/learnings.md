...
### 2026-05-27 — process — /dependabot-pr should surface adjacent available updates via `cargo update` inspection

**What happened:** During a `/dependabot-pr 574` round handling a `cargo-deps` group bump (typetag 0.2.21 → 0.2.22; serde_json 1.0.149 → 1.0.150), the user pointed out: the skill currently inspects only the diff Dependabot prepared (lockfile-only scope, classification, CI matrix). It does NOT inspect what other updates are available in the workspace at the same time. Running `cargo update` (dry-run shape: `cargo update --dry-run` or the existing `cargo update` then `git diff Cargo.lock` to see what would change) and `cargo update --verbose` would reveal additional minor updates that could be batched with the Dependabot PR's bumps, and would surface major-version updates currently held back by the `^x.y.z` semver rules that the user could then opt into manually (per AGENTS.md `## Dependency Versions` — `0.x` / `x` constraints stay; major bumps are user-driven).
...
### 2026-05-30 — process — subagent prompt undercounted assert_matches! sites; design AC was authoritative
**What happened:** During `/task` #584 Step 8 Group A subtask 3 (convert `assert!(matches!(...))` → `assert_matches!` in quartzite-paint-api), the subtask prompt said brush.rs had "2 sites (~305/312)" and "(No message form in this crate.)". Reading the code showed brush.rs actually had FIVE test-code sites: two plain (305/312) PLUS three message-form `assert!(matches!(v, P), "msg")` (lines 265, 284, 319). The design Decomposition table also said "2 sites 305/312". The spec AC4 ("Every `assert!(matches!(...))` in test code … converted") and the design's own translation rules (which explicitly cover the message form) were the authoritative contract, so I converted all five — message form → `assert_matches!(v, P, "msg")`. Leaving the three would have failed AC4 and tripped subtask 11's residual `rg 'assert!\(matches!'` defect grep.
**Rule:** When a subtask prompt's enumerated line list or "no X form here" claim contradicts the actual source, verify against the source with `rg`/Read and follow the spec/design AC (the binding contract), not the prose count. The "~" prefix on line numbers signals approximate; treat counts as hints, never as the exclusion boundary. Convert by category (plain/guard/message) across every test-code hit, not just the listed lines.
**Kind:** correction
**Escalated?** agent:design

### 2026-05-30 — testing — `assert_matches!` requires `Debug` on the scrutinee; `assert!(matches!(...))` does not — blocks #584 subtask 6
**What happened:** During `/task` #584 Step 8 Group B subtask 6 (convert 7 `assert!(matches!(w.widget_view(), WidgetView::X(_)))` sites in quartzite-widgets to `assert_matches!`), all 7 conversions failed to compile with E0277: `WidgetView<'_>` does not implement `Debug`. `std::assert_matches!` formats the actual value with `{:?}` on mismatch, so the scrutinee type MUST be `Debug` — whereas `assert!(matches!(...))` never formats the value and imposes no such bound. `WidgetView<'a>` is a public `#[non_exhaustive]` enum holding `&'a Button`/`&'a Label`/… and `Other(&'a dyn AsWidget)`; none of the concrete widgets derive `Debug`, and `AsWidget` has no `Debug` supertrait. Adding `#[derive(Debug)]` would require `Debug` on every concrete widget + a `Debug` supertrait on `AsWidget` — a large user-facing production API change, explicitly out of scope (spec: "modifies test assertions rather than adding behaviour … not a feature-code migration"). Reverted all 7 files (`git checkout --`); tree clean, `cargo build` green. Subtasks 4 (style-dispatch, 6 sites) + 5 (core, 3 sites) committed clean — their scrutinees (`PaintEvent`, `Value`) DO derive `Debug`. Subtask 6 NOT committed; blocker escalated to orchestrator.
**Rule:** Before adopting `assert_matches!` for a site, confirm the scrutinee type implements `Debug` (it is a hard macro requirement that `assert!(matches!())` lacks). A "mechanical, test-only" `matches!`→`assert_matches!` migration is NOT universally mechanical: any scrutinee whose type lacks `Debug` cannot convert without a production `Debug` impl. Design/spec enumerating conversion sites should verify `Debug` on each scrutinee type, not just the assert form. When conversion would force an out-of-scope production change, leave the `assert!(matches!(...))` as-is and surface to the orchestrator (spec/design amendment) rather than expanding scope unilaterally.
**Kind:** correction
**Escalated?** AGENTS.md, agent:design, agent:self-review, agent:review-findings

### 2026-05-30 — testing — design undercounted default_style_tests.rs assert_matches! sites by 15× (3 → 45)
**What happened:** During `/task` #584 Step 8 Group B subtask 9, the design Decomposition table estimated `quartzite-style/src/default_style_tests.rs: 3` `assert!(matches!(...))` sites (single-line 415/565/566). The file actually contained **45** convertible test-code sites — the other 42 are multi-line **message-form** `assert!(\n    matches!(scrutinee, Pattern if guard),\n    "msg"\n)` blocks that a single-line `rg 'assert!\(matches!'` misses. AC4 ("EVERY test-code `assert!(matches!(...))`") is the binding contract, so all 45 were converted (→ `assert_matches!(scrutinee, Pattern if guard, "msg")`). Non-convertible `assert!(<bool expr containing matches!>)` forms — `assert!(!painter.events.iter().any(|e| matches!(...)))`, `assert!(painter.events.iter().any(|e| matches!(...)))` — were correctly excluded (their first arg is a boolean expression, not a bare `matches!` call). Used a paren-matching Python transform (unwrap `matches!(...)` → `assert_matches!`) to avoid 45 hand-edits, then verified 0 remaining + `cargo test -p quartzite-style` (105 lib tests pass) + `cargo fmt --check` + clippy clean. Scrutinee `&PaintEvent` (local enum, `#[derive(Debug)]`) satisfies the Debug requirement.
**Rule:** When counting `assert!(matches!(...))` sites for an `assert_matches!` migration, the multi-line split form `assert!(\n matches!(...)\n, "msg")` is the dominant form in message-carrying assertions and is invisible to a naive single-line grep. Use a multiline-aware scan (`rg -U 'assert!\(\s*matches!'` or a paren-matcher) and classify per-site: convert only when the assert's FIRST argument is a bare `matches!(...)` call; leave `assert!` of any larger boolean expression (`!`, `&&`, `.any(|..| matches!)`) untouched. Treat design counts as hints; AC4 is the boundary.
**Kind:** correction
**Escalated?** AGENTS.md, agent:design

### 2026-05-30 — testing — Group B subtasks 7 (runtime) & 8 (renderer) blocked by non-Debug scrutinees — same class as subtask 6
**What happened:** During `/task` #584 Step 8 Group B, subtasks 7 and 8 each contain sites whose scrutinee type lacks `Debug`, so `assert_matches!` conversion fails E0277 (empirically confirmed per site, then reverted). **Subtask 7 (quartzite-runtime):** `restore_tree()` → `Result<(ObjectTree, ObjectId), _>` where `ObjectTree` (holds `SlotMap<_, Box<dyn Object>>`) is NOT Debug (E0277 confirmed); `restore_object()` → `Result<Box<dyn Object>, _>` where `dyn quartzite_core::Object` is NOT Debug (the `Object` trait has no Debug supertrait; E0277 confirmed). 4 of the 5 runtime sites (tree.rs:450, object.rs:316/332, tests/snapshot.rs:451) are blocked; only `capture_object()` → `ObjectSnapshot` (Debug ✓) at object.rs:421 would convert. NOTE: the runtime `snapshot` module is `#[cfg(feature = "serde")]`-gated, so it is invisible to a default-feature `cargo test -p quartzite-runtime`; probe with `--features serde`. **Subtask 8 (quartzite-renderer):** the 4 `vello_painter.rs` sites match on `LocalBrushKind<'_>`, a crate-private helper enum with NO derive at all (NOT Debug; E0277 confirmed). The 2 `render_harness.rs` sites (`RendererError`, Debug ✓) and 4 `event_convert.rs` sites (`MouseEventKind`/`KeyEventKind`, Debug ✓) WOULD convert. Both subtasks left fully reverted (no partial commit), surfaced to orchestrator for a scope decision: a runtime `Object`/`ObjectTree` Debug cascade (large, like the widget one) for subtask 7, and a one-line `#[derive(Debug)]` on the private `LocalBrushKind` for subtask 8 (small, but still a scope call the orchestrator owns).
**Rule:** The Group B `assert_matches!` migration is NOT uniformly mechanical — re-confirm `Debug` per scrutinee even after Group A's widget Debug cascade. `Result<T, E>` needs BOTH `T: Debug` and `E: Debug`; `Box<dyn Trait>` needs the trait to carry a `Debug` supertrait; tuple/struct Ok-types need their own derive. Feature-gated test modules (runtime `snapshot` behind `serde`) must be probed with the gating feature enabled or the conversion silently "compiles" because the module is excluded. When any site in a subtask's file set is blocked, revert the WHOLE subtask (no partial commit) and surface — do not add `Debug` to production/private types unilaterally.
**Kind:** correction
**Escalated?** AGENTS.md, agent:design, agent:self-review, agent:review-findings

### 2026-05-30 — testing — pre-existing `--all-features` clippy breakage in quartzite-core surfaced during #584 Group A subtask 8
**What happened:** Running the subtask-8 prescribed gate `cargo clippy --workspace --all-targets --all-features -- -D warnings` failed with 25–28 errors in `quartzite-core/src/{snapshot/tree.rs,value.rs}` (`use_self`/`unnecessary structure name repetition`, `BTreeMap::default()` clarity, `single_char_pattern`, `const fn`). Confirmed PRE-EXISTING and UNRELATED to Group A: stashing my single subtask-8 edit (`quartzite-runtime/src/snapshot/object.rs`) and re-running on clean HEAD `7e49ca3` reproduced the same 25 `quartzite-core` lib errors. The lints live in a `serde`/`--all-features`-gated code path of quartzite-core, a crate NOT touched by subtasks 6/7/8. The default-feature `cargo clippy --workspace --all-targets -- -D warnings` is GREEN, so Group A introduces no clippy regression. Subtask 8's functional gates (`cargo build --features serde -p quartzite-runtime`, `cargo test -p quartzite-runtime --features serde`, fmt, default clippy) all pass; committed on that basis.
**Rule:** When a subtask's `--all-features` gate fails in a crate the subtask never touched, STOP and confirm pre-existence (stash the change, re-run on clean HEAD) before treating it as a regression. A confirmed pre-existing `--all-features` failure is a Group C (subtask 13 full gate matrix incl. `cargo test --all-features`) blocker to be surfaced to the orchestrator — NOT a reason to block the current in-scope subtask whose own gates pass, and NOT to be fixed unilaterally inside an unrelated subtask. Group C MUST address the quartzite-core `--all-features` clippy debt before the terminal gate can go green.
**Kind:** correction
**Escalated?** agent:design

### 2026-05-30 — testing — second abort-masked debt layer surfaced after clearing the 35-site clippy debt (#586 subtask 4)
**What happened:** After clearing all 35 enumerated `--all-features` clippy sites (28 quartzite-core in Group A + 7 quartzite-runtime in subtask 4), the AC1 workspace gate `cargo clippy --workspace --all-targets --all-features -- -D warnings` STILL did not exit 0. Two further layers surfaced, each masked behind the previous abort: (1) 2 additional clippy sites in `quartzite-runtime/tests/snapshot.rs` (`use_self` line 60, `default_trait_access` line 444) — same crate / same lint classes as the 7 designed `src/` sites, NOT enumerated in the design (which verified only `src/snapshot/{object,tree}.rs`); I fixed these as clearly within subtask 4's intent. (2) 51 `missing '///' documentation` HARD ERRORS emitted by the `#[object_impl]` proc-macro under `--all-features` across `quartzite-macros` (tests object_impl/via_facade/object/extend), `quartzite-style` (test third_party_paint), `quartzite` (example combined, test signal_to_signal, bench macro_object), `benches/macro_object.rs`, `examples/hello_object.rs`. Confirmed PRE-EXISTING and OUT OF SCOPE: `git checkout e8dc197` (branch base / master) + `cargo clippy -p quartzite-macros --test object_impl --all-features` reproduced 12 of them on clean base. These are macro-emitted doc-enforcement errors (not clippy lints, not in the 35-site spec count, in crates the task never touched).
**Rule:** Clearing one abort-masked debt layer can reveal the NEXT layer the same compile-abort was hiding — re-running the terminal gate after a cleanup can surface a fresh, larger, out-of-contract class. When it does: (a) fix only what is unambiguously within the current subtask's crate+intent (here: the 2 sibling `tests/snapshot.rs` sites), (b) STOP before expanding into the new class, (c) confirm pre-existence on the branch base, and (d) surface it to the orchestrator as an AC1 blocker requiring a Spec/Design Amendment — do NOT silently absorb a multi-crate macro-doc debt class into a clippy-cleanup subtask. The `#[object_impl]` `undocumented` hard-error under `--all-features` is a distinct debt class from the clippy `-D warnings` debt and needs its own scoping decision.
**Kind:** correction
**Escalated?** agent:design
...
**Kind:** correction
**Escalated?** AGENTS.md, agent:self-improve (rule in ai-docs/workflow.md § Dependent tool calls must not be batched…)
...
### 2026-05-31 — process — repeated the batch-commit slip during the very /improve run that escalated it
**What happened:** In the `/improve` run (PR #599) that escalated the no-batch-dependent-calls rule, I twice batched `Edit` calls with a `git commit` in the same turn. (1) First apply turn: I built the `Edit` `old_string`s from the self-improve subagent's *proposed* diffs rather than the files' actual text, so most anchors missed; the batched commit ran against a near-empty stage and produced "nothing to commit" — no over-claim, but a wasted turn. (2) A six-`Edit` + commit batch with the same proposed-text problem. The escalated rule was authored from these exact failures; ironically I kept hitting it until I slowed to one careful Edit→verify→commit sequence using each file's real text.
**Rule:** Already escalated (AGENTS.md § Workflow + ai-docs/workflow.md § Dependent tool calls must not be batched + self-improve.md Step 5). Extra nuance worth holding: when applying a subagent's proposed diffs, the proposed `old_string` is a *paraphrase risk* — always Read the target's actual text before constructing the `Edit`, never copy the subagent's quoted "old text" verbatim. Apply edits, await results, `git diff --cached --stat`, THEN commit — each in its own turn.
**Kind:** correction
**Escalated?** AGENTS.md, agent:self-improve (rule already in ai-docs/workflow.md § Dependent tool calls must not be batched…; this entry adds the proposed-diff paraphrase-risk nuance, no new escalation)

### 2026-06-01 — process — surfaced an already-`untracked` deferred row as a fresh sweep candidate (manual read vs jq filter)
**What happened:** During an orchestrator-driven `/triage` full sweep, I curated ci-docs-workflow follow-ups by manually reading the 358-line JSONL in slices and recording line numbers. I flagged the "Variant-B callout drift" row (line 36) as a `tracked=="—"` candidate and offered it for promotion; it was actually already `tracked=="untracked"` at HEAD (declined in a prior triage). The user approved promoting it, so I created issue #619 and re-promoted the row (overwrote `untracked`→`#619`) — net outcome consistent with the user's choice, but the row should never have entered the candidate set. Caught only at writeback when the `"—"` Edit anchor failed.
**Rule:** The sweep candidate set is defined by the live `jq -c 'select(.tracked=="—")'` filter, NOT by manual line-reading. When curating a large file, derive the candidate list programmatically (jq) and draft titles from THAT set; never promote a row to the approval prompt based on a hand-read line whose `tracked` value was not re-confirmed against the jq filter. Manual reads of large JSONL are for understanding content, not for determining `tracked` state.
**Kind:** correction
**Escalated?** no

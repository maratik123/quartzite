...
- **When required:**
    ...
    - **Methods inside `impl<T> ...` or `impl<T> Trait for Foo<T>` blocks
      ...
      compiler's point of view. Place `/// _Simple._` directly above the
      `fn` keyword inside the impl block — this is allowed despite the
      *Trait-impl exemption* below (the exemption removes the *requirement*
      to satisfy the full convention, not the right to add a doc tag).
      Examples: `Signal<Args>::default`, `ObjectRef<T>::clone`,
      `WeakRef<T>::eq`.

...
| `quartzite-runtime` | `Application` (Object, builder pattern via `ApplicationBuilder`), `EventLoop` (Object, tickless by default; `Option<Duration>` tick policy), `ObjectTree`, `ObjectRef`, `Timer`, `ObjectTreeExt`, `try_with_tree` |
...
| `quartzite-style-dispatch` | Bridge crate: `dispatch_paint` free function + `WidgetResolver` trait (immutable, paint-time). Walks widget tree depth-first, calls `Style::draw_widget` once per visible node; `TranslateGuard::with_clip` (from `quartzite-paint-util`) for per-widget clip-rect (reads `AsWidget::children_clip_rect()` on parent; order: `save → clip_rect(parent-local rect) → translate → visit → restore`); `tracing::warn!` on resolver miss. Depends on `quartzite-core`, `quartzite-paint-api`, `quartzite-paint-util`, `quartzite-style`, `quartzite-widgets` — no `quartzite-renderer` dep (no cycle). |
| `quartzite-widgets` | WidgetBase, WidgetExt, Layout (`BoxLayout`, `GridLayout`), `Label`, `Button`, `LineEdit`, `TextEdit`, `ScrollArea`, `Container` — widget system ✅ (#46). `AsWidget::children_clip_rect(&self) -> Option<Rect>` default-None (emitted by `#[derive(Extend)]` macro at `WidgetBase` root); `ScrollArea` overrides via `#[clip_rect(method = "content_rect")]` annotation to return `Some(content_rect())`; `content_rect()` returns zero-inset local-frame rect (v1; insets for chrome land with #315). |
...
| `EventLoop` tick policy | `tick: Option<Duration>` field: `None` → tickless (`recv()` blocks until a message arrives — zero CPU waste); `Some(d)` → tick-based (`recv_timeout(d)`). `Some(ZERO)` normalised to `None`. Configured via `EventLoop::with_tick(Option<Duration>)` / `ApplicationBuilder::tick_duration`. Default is tickless. |
| `ApplicationBuilder` / `Application::new()` removal | `Application::new()` deleted; replaced by builder pattern: `Application::builder() -> ApplicationBuilder` (const fn); `ApplicationBuilder::build() -> Result<Application, ApplicationError>`. `ApplicationBuilder::tick_duration(Option<Duration>)` propagates to the inner `EventLoop`. `WindowedApplicationBuilder` extended with the same `tick_duration` setter. |
| `EventLoop` + `Application` Object-ification | `EventLoop` uses `#[derive(Extend, Object)] #[root]` + `#[base] pub base: ObjectBase` + `#[object_impl]`; `#[slot] pub fn stop(&mut self)` for reflection + `pub fn request_stop(&self)` shim for `Arc<EventLoop>` cross-thread callers. `Application` uses hand-rolled b1 (`Application(Arc<ApplicationInner>)` tuple-struct can't use the derive macro); `object_base_mut` always panics (Arc prevents mutable borrow on a shared handle); `pub fn quit(&self)` works from any context — closures, signal connections, and `invoke_method` dispatch (coerces `&mut Application` → `&Application` at call site). `SlotEntry::callback` widened to `Box<dyn Fn + Send + Sync>` so `Arc<EventLoop>: Send` holds. |

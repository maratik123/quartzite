...
| `ObjectBase::signals_blocked` | Private `bool` field; toggled via `block_signals()` / `unblock_signals()`. Generated `emit_<signal>` wrappers and property notify emissions call `Signal::emit_checked(signals_blocked(), args)` — suppressed when blocked. `Signal::emit_unchecked` is the low-level primitive that intentionally bypasses the check. |

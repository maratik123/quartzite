...
- **const-default-fn** — replaced `#[derive(Default)]` (or hand-written `Default` impls) with explicit `impl Default for T { fn default() -> Self { Self::new() } }` delegating to a new `pub const fn new() -> Self` on 4 Group (A) types: `Path` (`quartzite-paint-api`), `CloseEvent` (`quartzite-events`), `Palette` (`quartzite-style-types`), `DefaultStyle` (`quartzite-style`). Callers gain a `const fn` path usable in `const`/`static` initialisers. 18 Group (B) types (atomics, `Arc`, `mpsc`, FFI) left untouched — zero liftable. Geometry types (`Point`, `PointF`, `Rect`, `RectF`, `Size`, `SizeF`, `Margins`) already conformant (multi-arg `pub const fn new(…)` already existed). `#[allow(clippy::derivable_impls, reason = "…")]` per impl site. [spec](plans/done/2026-05-19-const-default-fn.spec.md)

...
| `quartzite-style` | Downstream `std` crate: `Style` trait, `StyleRegistry`, `DefaultStyle` (zero-sized concrete impl; flat fill/1-px-outline rendering for Button/Label/TextEdit/ScrollArea; opt-in registration). Re-exports `Palette` and `ColorRole` from `quartzite-style-types`. |

...
27. Tickless `EventLoop` + `Application`/`EventLoop` Object-ification ✅ (#561). `EventLoop` tick policy: `Option<Duration>` (`None` = tickless, default; `Some(d)` = tick-based, `ZERO` normalised to `None`). `Application::new()` removed; replaced by `ApplicationBuilder` builder pattern; `WindowedApplicationBuilder` extended with `tick_duration`. `EventLoop` + `Application` Object-ified: `EventLoop` via `#[derive(Extend, Object)]` + `#[slot] stop`; `Application` hand-rolled (b1 — Arc<Inner> shape); `request_stop`/`request_quit` shims for ergonomic `&self` callers. `SlotEntry::callback` widened to `Send + Sync`.

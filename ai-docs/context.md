...
| `quartzite-style` | Downstream `std` crate: `Style` trait (with `caret_visible_now` + `prefers_reduced_motion` methods), `StyleRegistry`, `StyleClock` (blink-phase helper; `StyleClock::pinned` for snapshot tests), `DefaultStyle` (clock-bearing struct impl; flat fill/1-px-outline rendering for Button/Label/TextEdit/ScrollArea/Container/LineEdit; `paint_caret`/`paint_selection` helpers for TextEdit; `start_blink_timer` behind `runtime-blink` default-on feature; opt-in registration). Re-exports `Palette`, `ColorRole`, and `ColorGroup` from `quartzite-style-types`. |

...
| `quartzite-style-dispatch` | Bridge crate: `dispatch_paint` free function + re-exported `WidgetResolver` trait (immutable, paint-time; the trait itself now lives in `quartzite-hit-test` per #395 and is re-exported here via `pub use quartzite_hit_test::WidgetResolver;` so every existing import path stays valid). Walks widget tree depth-first, calls `Style::draw_widget` once per visible node; `TranslateGuard::with_clip` (from `quartzite-paint-util`) for per-widget clip-rect (reads `AsWidget::children_clip_rect()` on parent; order: `save → clip_rect(parent-local rect) → translate → visit → restore`); `tracing::warn!` on resolver miss. Depends on `quartzite-core`, `quartzite-paint-api`, `quartzite-paint-util`, `quartzite-style`, `quartzite-widgets`, `quartzite-hit-test` — no `quartzite-renderer` dep (no cycle). |
| `quartzite-hit-test` | Paint-free crate (#395): `hit_test(root: ObjectId, point: Point, resolver: &dyn WidgetResolver) -> Option<(ObjectId, Point)>` — the structural inverse of `dispatch_paint`. Walks the widget tree depth-first but iterates children in **reverse** and tests child-before-parent, so the visually-topmost (last-painted) widget wins. Returns the hit id + accumulated origin offset (root-local → hit-local). Same visibility-skips-subtree, `children_clip_rect()`, and resolver-miss `warn!` semantics as paint. Owns the immutable `WidgetResolver` trait + closure blanket impl (moved out of `quartzite-style-dispatch`). Depends only on `quartzite-core`, `quartzite-geometry`, `quartzite-widgets`, `tracing` — touches no `Painter`/`Style`/`Palette`. **Hit-test traversal primitive only**; the event-dispatch loop (routing `MouseEvent` to the hit widget) is a separate follow-up. Not wired into the `quartzite` umbrella facade. |
...
Feature plans (post-crate-level, in implementation order). Full per-plan summaries live in [`plans-summary.md` § Dependency-order feature plans](plans-summary.md#dependency-order-feature-plans--reference):

13. `DefaultStyle` GPU snapshot tests ✅ (#297)
14. `Button` hover/pressed/focused visual states ✅ (#316)
15. `quartzite-style-dispatch` bridge crate ✅ (#312, #393)
16. `DefaultStyle` Container + LineEdit arms ✅ (#318)
17. `DefaultStyle` read-only overlay fix ✅ (#458)
18. `ColorGroup` axis + `FocusRing` role ✅ (#402)
19. `Label` / `TextEdit` / `ScrollArea` hover/pressed/focused visual states ✅ (#403)
20. `LineEdit` hover/pressed/focused visual states + disabled-axis parity ✅ (#406, #407)
21. `WidgetExt::paint` collapsed into `Style::draw_widget` ✅ (#409)
22. `quartzite-paint-util` RAII transform guard ✅ (#410)
23. `signals_blocked` serde persistence ✅ (#39)
24. `TextEdit` caret + selection rendering ✅ (#317)
25. `LineEdit` caret + selection rendering ✅ (#405)
26. Design-system conformance audit + vertical-alignment paint-API axis ✅ (#555)
27. Tickless `EventLoop` + `Application`/`EventLoop` Object-ification ✅ (#561)
28. `WindowedApplication::new()` shorthand ✅ (#567)
29. Per-widget clip-rect (`ScrollArea` content clipping) ✅ (#397)
30. `quartzite-hit-test` paint-free hit-testing ✅ (#395)

...
| `quartzite-style` | Downstream `std` crate: `Style` trait (`Send + Sync`, generic-only `draw_widget(&dyn AsWidget, &mut dyn Painter, &Palette)`), `StyleRegistry` (`OnceLock<Mutex<Option<&'static dyn Style>>>` storage with `Box::leak`, non-panicking `try_style() -> Option<&'static dyn Style>`, lock-poisoning recovery via `unwrap_or_else(\|e\| e.into_inner())`), `DefaultStyle` (zero-sized concrete impl; flat flat/fill/1-px-outline design for Button/Label/TextEdit/ScrollArea; opt-in registration). Re-exports `Palette` and `ColorRole` from `quartzite-style-types`. |
...
- **cleanup-progress-issue-derive** — `.claude/skills/pr-merged/scripts/cleanup-progress.sh` was deriving the spec-lookup key from the merged **PR** number, but `/task` + `/interview` write the **issue** number into `**Tracked in:**`. Fix: a `gh pr view` step extracts the issue number from the merged PR body's `Closes/Fixes/Resolves #N` line (case-insensitive, conjugations covered), then a tightened `grep -lE "^\*\*Tracked in:\*\* #<ISSUE_NUM>\b"` looks up the matching spec (the anchor on `^**Tracked in:** ` prevents prose-line false-matches). Both miss cases now emit one-line stderr warnings instead of silently no-op'ing; idempotent re-runs preserved. Latent bug since PR #300; recipe + AC4 fixture (PR #172) added to the design. [spec](plans/done/2026-05-13-cleanup-progress-issue-derive.spec.md)
- **shrink-agents-md** — `AGENTS.md` reduced from 40,572 chars to ~31,466 chars (~20% headroom under the 40k perf threshold). AXIOM-vs-redundant-prose dedup across `## Workflow`, `## Dependency Versions`, `## Propagation Rule`; narrative extraction into two new reference pages `ai-docs/workflow.md` (PR review comment resolution recipe) and `ai-docs/corrections-log.md` (Boundary rule 1 / 2 Exception bodies + Entry-format field glossary). All 8 AXIOMs preserved verbatim; Propagation sync-group lookup table preserved in AGENTS.md. [spec](plans/done/2026-05-13-shrink-agents-md.spec.md)

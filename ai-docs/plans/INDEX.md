...
1. **md ↔ gh issues bridge in `/triage` (#205, C of process-improvements)** — B (#204 ✅) just landed `/triage` with the promotion + drain flows and a *Bridge* placeholder section. C extends `triage/SKILL.md` with the drift-detection logic: bulk `gh issue list` reused; iterate `Tracked` cells; flag closed-issue refs / status mismatches; per-conflict user resolution; pagination watchdog. `_inbox.md` `—` rows excluded from the sweep (drain step is canonical, carved out in B). Blocked-label clears on B merge; pick this up next via `/task 205` — the final umbrella issue of the process-improvements meta-plan.

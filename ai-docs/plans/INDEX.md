...
1. **`/triage` skill (#204, B of process-improvements)** — A2 (#203 ✅) just landed `_inbox.md` with 294 backfill rows + the parser. B creates the `/triage` skill with opus subagent, drains `_inbox.md` per-entry (sort/promote/drop), and promotes untracked rows in the 8 thematic files + `widget-backlog.md` 🟡 v2 rows to gh issues. Blocked-label clears on A2 merge; pick this up next via `/task 204`.

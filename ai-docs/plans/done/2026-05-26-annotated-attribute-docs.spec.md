...
4. Rename the macro attribute identifiers to match the issue body's vocabulary (clean break, no compat alias):
   - `#[prop]` → `#[property]`
   - `#[invokable]` → `#[invoke]`
   Every use-site in workspace `src/`, `examples/`, `tests/`, and proc-macro internals (`quartzite-macros/src/**`) is updated in the same PR.
5. Extend `ai-docs/doc-convention.md` with a new section (`## Annotated items`) that names the convention: annotated items always carry `///` docs even when private; the existing pub-only exception in § *Scope* does not apply when any of the inventoried attributes is present on the item. The section uses the post-rename attribute names (`#[property]`, `#[invoke]`).
6. Extend `ai-docs/code-style.md` with a sibling section (`## Annotated items`) that names the visibility-lift default: every annotated item is `pub` unless its companion `// why-private:` comment defends the exception. Defence-in-depth: the doc-convention requirement (point 5) survives a future `why-private:` exception without losing the doc requirement.
7. Update the proc-macro crate (`quartzite-macros/src/{object,object_impl,object_part,extend}/`) so each macro emits a **tri-state diagnostic** (`allow` / `warn` / `deny`) at expansion time when an annotated item is missing its `///` doc. The semantics mirror rust's native lint-level vocabulary:
...
- Adding net-new attribute identifiers to the macro vocabulary beyond the two renames in scope item 4. No new attributes (e.g. an additional `#[notify]` alongside the existing `notify` sub-attribute) are introduced.
...
- Making `deny` the default — the default remains `warn`. `deny` is opt-in only, per scope item 7. (Workspace CI MAY enable `deny` via the global cargo feature, but the spec does not mandate that decision.)

## Deferred

- Whether `#[property]` sub-attributes (`notify` / `read_only` / `constant` / `stored` / `designable` / `user`) deserve a per-sub-attribute doc-comment paragraph template | low-leverage, design phase can choose | no separate issue needed
...
Attributes recognised by the workspace proc-macros (`quartzite-macros/src/lib.rs`). Names below reflect the **post-rename** state delivered by this task; the pre-rename names (`#[prop]`, `#[invokable]`) survive only in the rename-sweep AC (AC6) and the rename mapping below.
...
**Rename mapping (clean break, no compat alias):**
...
All `#[signal]` / `#[slot]` / `#[invoke]` / `#[property]` sites observed in workspace library code are already `pub` (post-rename) — the visibility-lift workload concentrates on the `#[base]` / `#[widget_children]` pattern above.
...
| Macro attribute renames (`#[prop]` → `#[property]`, `#[invokable]` → `#[invoke]`) | Rename both — clean break, no compat alias. Every use-site in workspace `src/` + `examples/` + `tests/` + `quartzite-macros/src/**` updated in the same PR (per AGENTS.md § *API Stability*: pre-publish, no shims). |
...
- Renaming `#[prop]` → `#[property]` and `#[invokable]` → `#[invoke]` is a clean break — no compat alias retained. The use-site sweep covers `src/`, `examples/`, `tests/`, and `quartzite-macros/src/**` in the same PR. AGENTS.md § *API Stability* (pre-publish, no shims) sanctions the break.
...
| AC6 | Every `#[prop]` / `#[invokable]` use-site in workspace `src/`, `examples/`, `tests/`, and `quartzite-macros/src/**` is renamed to `#[property]` / `#[invoke]` in the same PR. No compat alias retained; `grep -rn '#\[prop\b' --include='*.rs' .` and `grep -rn '#\[invokable\b' --include='*.rs' .` both return empty. |
...
- Whether the `#[signal]` / `#[slot]` / `#[invoke]` / `#[property]` summary lines deserve a per-attribute rustdoc template (e.g. "Signal emitted when …" / "Slot invoked when …") in `doc-convention.md` § *Annotated items*, or whether a single common template suffices. Sensible default: single common template; revisit if reviewers want per-attribute prose.

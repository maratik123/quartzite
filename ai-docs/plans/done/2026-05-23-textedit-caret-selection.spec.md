...
| AC5 | `DefaultStyle::paint(&TextEdit, …)` with a single-line selection (no wrap) paints exactly one selection `fill_rect` whose horizontal span is `(sel_start_x, sel_end_x)` and whose vertical span is one line's `line_height`. The selection `fill_rect` is emitted *after* the main `draw_text_in` (covering already-drawn normal-coloured glyphs in the selection region); a clipped `draw_text_in` overdraw then re-renders highlighted glyphs on top. A `RecordingPainter` unit test asserts the fill-rect presence and its span. |

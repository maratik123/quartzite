...
| # | Criterion |
|---|-----------|
| AC1 | For every type identified as Group **(A)** in the design's inventory (including struct-shaped enums with a `#[default]` data-carrying variant — pure unit-variant enums are out of scope): `#[derive(Default)]` is removed (or the hand-written `Default` impl is rewritten), a `pub const fn new() -> Self` (zero-arg) exists with `#[inline]` + `# Examples`, and `impl Default for T` delegates to `Self::new()`. Types already exposing a multi-arg `pub const fn new(…)` (geometry types, `Color`, `Pen`, etc.) are already conformant per Scope item 4 and are explicitly excluded from this AC. |

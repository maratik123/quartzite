...
1. `self-improve` reads `~/.claude/projects/-home-syt-RustroverProjects-quartzite/memory/feedback_*.md` and `MEMORY.md` (index) but writes nothing to that directory.
...
- `~/.claude/projects/-home-syt-RustroverProjects-quartzite/memory/` — the user-local auto-memory directory Step 1c reads. Currently contains `MEMORY.md` (index) + 10 `feedback_*.md` files (currently) plus 1 `project_*.md` pinning entry.

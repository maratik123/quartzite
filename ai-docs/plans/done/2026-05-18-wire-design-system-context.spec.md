...
4. **`.claude/skills/ui-design/SKILL.md`** — new file (~10 lines). Frontmatter (`name: ui-design`, short description) + minimal body that instructs the agent to Read `design-system/SKILL.md`, then `design-system/README.md`, then explore `design-system/preview/` and `design-system/ui_kits/widgets/` as needed. Pointer only — no inlined content.
...
- **Slug uniqueness:** confirm `.claude/skills/ui-design/` does not already exist before creating it; the existing skill list shows no `design` slug today, but design phase must re-verify.
...
| AC5 | New file `.claude/skills/ui-design/SKILL.md` exists. Frontmatter has `name: ui-design` + short description. Body is ~10 lines instructing the agent to Read `design-system/SKILL.md`, then `design-system/README.md`, then explore `design-system/preview/` and `design-system/ui_kits/widgets/` as needed. Verify: slash-command dispatcher loads the new skill and returns its body. |

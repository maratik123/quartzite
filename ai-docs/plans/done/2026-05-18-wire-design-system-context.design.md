...
|---|------|-------|------------|
| 1 | Add **Design system** pointer section to `AGENTS.md` (after `## Project`, before `## Permissions`), and add the new `.claude/skills/design/SKILL.md` row to § Agent Docs table. Sum of both additions ≤ 1 024 bytes. | `AGENTS.md` | — |
...
| 4 | Create new pointer skill `.claude/skills/design/SKILL.md` (~10 lines). Frontmatter `name: design` + short description per AC5. Body instructs Read `design-system/SKILL.md` → `design-system/README.md` → explore `design-system/preview/` and `design-system/ui_kits/widgets/` as needed. Pre-check: confirm `.claude/skills/design/` does not exist. | `.claude/skills/design/SKILL.md` (new) | 1, 2, 3 |
...
- **Group B:** subtask 4 — terminal group (1 subtask; within the `1..=3` range). Create the new `.claude/skills/design/SKILL.md` pointer file. Spawn `/context-reset` per `.claude/skills/context-reset/SKILL.md` § Compaction recovery (re-entry) at the entry into Group B.
...
- **`design-system/SKILL.md` `name: quartzite-design` vs. new skill `name: design`.** The existing `design-system/SKILL.md` declares `name: quartzite-design`. The new `.claude/skills/design/SKILL.md` declares `name: design` (per AC5 + spec § Key decisions). **Mitigation:** the two names are distinct; no collision. The new skill is a *pointer* — its body delegates to the existing skill by Read, not by `name:` alias. Document this in the new skill's frontmatter description so a future reader does not assume they are the same skill.
...
- `test -f .claude/skills/design/SKILL.md` — file exists.
- `head -5 .claude/skills/design/SKILL.md` — frontmatter has `name: design` + short description.
- `wc -l .claude/skills/design/SKILL.md` — body ~10 lines (loose check; AC5 says "~10 lines").

...
| `src/lib.rs:167` *(post-Step-12 amendment — Family A `github.com/.../tree/master/` URL)* | `//! See \`runtime::Timer\` and \`runtime::TimerConfig\`. A complete example //! lives at [\`examples/timer.rs\`](https://github.com/maratik123/quartzite/tree/master/examples/timer.rs).` | `//! See [\`runtime::Timer\`] and [\`runtime::TimerConfig\`].` Drop the second sentence + the repo-tree URL. **No information loss:** `Cargo.toml`'s `[[example]] name = "timer"` declaration causes `cargo doc` to surface the example via the rendered *Examples* section on docs.rs automatically; downstream readers retain discoverability through rustdoc's own example-surfacing path rather than through an internal `github.com/.../tree/master/` URL. The remaining first sentence is upgraded to intra-doc links (`[\`runtime::Timer\`]` / `[\`runtime::TimerConfig\`]`) consistent with the rest of the module-level doc. |
...
**Post-Step-12 Pattern A token broadening.** The original Pattern A regex GitHub-URL token `github\.com/.+/(issues|pull)/` missed `/tree/master/` subpaths (and by inference `/blob/`, `/commit/`, `/raw/`). User feedback on PR #516 surfaced one such leak at `src/lib.rs:167` (pre-amendment line numbering) which cited `https://github.com/maratik123/quartzite/tree/master/examples/timer.rs`. The spec was amended to broaden the alternation to `(issues|pull|tree|blob|commit|raw)/` (9-char regex delta — 5 new alternation chars + 4 separators). The broadening preserves the existing AC1/AC7 contract because the **bare** `https://github.com/maratik123/quartzite` install URL at `src/lib.rs:192` (inside a Cargo.toml code fence) has no subpath following the repo segment and continues to be ignored by the regex — the trailing `/` after the alternation group is mandatory, so `quartzite` followed by closing `"` does not match. Three files synced (script, convention, spec) to keep the canonical-source contract intact; one file rewritten (`src/lib.rs:167` sentence drop).

## Post-Step-12 spec amendment

Recorded for audit / future-recurrence diagnostic. The amendment landed AFTER `/task` Step 12 (spec was moved to `done/`), so the round count below restarts from a fresh round rather than counting against the original 3-round design cap.

- **Trigger.** PR #516 self-review Round 2 / user feedback identified that `src/lib.rs:167` cited `https://github.com/maratik123/quartzite/tree/master/examples/timer.rs`. The narrower Pattern A GitHub-URL alternation `github\.com/.+/(issues|pull)/` did not catch `/tree/`, `/blob/`, `/commit/`, or `/raw/` subpaths — a gap that the post-rewrite Pattern A re-run (subtask 1 verification) silently passed because the URL did not match.
- **Spec change.** Pattern A token alternation extended by `tree|blob|commit|raw` (5 alternation chars + 4 separators = 9-char regex delta). The post-Step-12 parenthetical was appended to the Pattern A header text in the spec's `## Audit patterns` section. AC1 holds unchanged — the regex stays in sync across `scripts/check-rustdoc-internal-refs.sh` + `ai-docs/doc-convention.md` + this spec (verified via `grep -n "issues|pull|tree|blob|commit|raw"` returning identical hits in all three files).
- **Code change.** Three files synced with the broadened regex (`scripts/check-rustdoc-internal-refs.sh`, `ai-docs/doc-convention.md`, the spec itself). One file rewritten (`src/lib.rs:167` — sentence drop per new replacement-prose row above). Net workspace-level effect: workspace-wide `rg 'github\.com/' --type rust` returns exactly 1 hit — line 192 (bare install URL inside a Cargo.toml code fence — Family A allowed because the URL is a worked example of API use, not a Family-A internal-artefact citation). The previous line 167 `/tree/master/...` hit is gone. _(Prior draft of this bullet claimed 2 hits; corrected post design-review.)_
- **Verification.** Post-amendment Pattern A run against the published surface returns zero matches; the broadening introduces zero false positives because (a) the bare install URL at line 192 has no path segment after `quartzite` and therefore does not match the trailing-`/` alternation; (b) no other workspace doc-comment cites a `github.com/<owner>/<repo>/<tree|blob|commit|raw>/` URL today (verified by `rg 'github\.com/.+/(tree|blob|commit|raw)/' --type rust`).
- **AC13 re-confirm (post-amendment).** The amendment touches `ai-docs/doc-convention.md` (Pattern A regex line — 9 chars longer). Re-measure: post-amendment `wc -c ai-docs/doc-convention.md` = 30,323 bytes (verified empirically on the working tree at the time of this design refresh), still comfortably under the 35k early-warning cap. The five-file post-edit projection table above (`### Review-gate edits` subsection) is **not** invalidated — the round-3 projection of 30,243 drifts by exactly 80 chars (the 9-char regex delta plus a one-line narrative addition the spec received), well within the 4.7k of remaining headroom. All four review-gate file sizes are unchanged.

## Decomposition

| # | Task | Files | Depends on |
|---|------|-------|------------|
| 1 | Rewrite all eleven published-surface doc-comment leaks per the replacement-prose table above: the eight Scope §3 entries + the two round-1-design-review additions (`snapshot.rs:138`, `snapshot/tree.rs:69`) + the post-Step-12 amendment add (`src/lib.rs:167` — `tree/master/` URL) + the in-table near-miss strip on `render_harness.rs:312–313` (`tests/support/mod.rs` parenthetical). **Subtask 1 ALSO rewrites the adjacent `#[error("…")]` attribute on `quartzite-core/src/snapshot.rs:134`** (sibling of the rewritten doc-comment on line 138 — same `\bthis implementation\b` token in `"unsupported schema version {found} (this implementation supports up to {supported})"`). Although Pattern B's `^\s*(///\|//!)` anchor does not match the `#[error(...)]` attribute line, the rendered `Display` output IS downstream-visible — `thiserror::Error::source()` surfaces it to callers — so AC4's "no information loss for downstream readers" principle applies identically. Sample reword: `"unsupported schema version {found} (supported up to {supported})"`. Re-run both audit patterns scoped to the published surface (with the post-Step-12 broadened Pattern A) and verify zero matches. | `quartzite-style-dispatch/src/dispatch.rs`, `quartzite-core/src/snapshot.rs`, `quartzite-core/src/snapshot/tree.rs`, `quartzite-widgets/src/widget_base.rs`, `quartzite-widgets/src/layout/mod.rs`, `quartzite-renderer/src/render_harness.rs`, `quartzite-style-types/src/dark_palette.rs`, `src/lib.rs` | — |
...
- **(Historical, now closed by post-Step-12 amendment) Pattern A GitHub-URL alternation was too narrow.** The original `github\.com/.+/(issues|pull)/` token missed `/tree/`, `/blob/`, `/commit/`, and `/raw/` subpaths. The `src/lib.rs:167` `tree/master/examples/timer.rs` URL leaked past subtask-1 verification on this basis. Resolution: post-Step-12 spec amendment broadened the alternation to `(issues|pull|tree|blob|commit|raw)/`; the three sync surfaces (`scripts/check-rustdoc-internal-refs.sh`, `ai-docs/doc-convention.md`, the spec's `## Audit patterns`) were updated in lockstep, and `src/lib.rs:167` was rewritten to drop the offending sentence. **Lesson for future audit-pattern design:** the regex must cover the full GitHub URL alternation set at definition time (issues + pull + tree + blob + commit + raw), not just the two highest-frequency forms. See *Post-Step-12 spec amendment* above for the full timeline.
...
- **Subtask 1 verification:**
  ...
  - **Rewrite-location enumeration for AC1 (10 sites + 1 near-miss strip).** All ten lines below must be rewritten per the replacement-prose table; the eleventh entry is an in-table-paragraph parenthetical strip co-located with site (5) and not a separately-pattern-caught hit:
    ...
    11. *(Post-Step-12 amendment.)* `src/lib.rs:167` — Family A — drop the second sentence (`A complete example lives at [\`examples/timer.rs\`](github.com/.../tree/master/examples/timer.rs).`) leaving `//! See [\`runtime::Timer\`] and [\`runtime::TimerConfig\`].`. `Cargo.toml`'s `[[example]] name = "timer"` declaration causes rustdoc to surface the example automatically — no information loss. Surfaces only under the broadened Pattern A regex (`(issues|pull|tree|blob|commit|raw)/`); the original `(issues|pull)/` regex did not catch it. See *Post-Step-12 spec amendment* above.

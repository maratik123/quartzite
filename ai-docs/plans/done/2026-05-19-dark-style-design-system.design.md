...
- **Compile-time check:** `const _: Palette = DARK_PALETTE;` as a top-level
  (non-`#[cfg(test)]`) item in `quartzite-style-types/src/dark_palette.rs` —
  fires even without `--cfg test`, making it a permanent regression guard.
  Satisfies AC3.

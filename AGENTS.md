...
- **Enum repr:** `#[repr(...)]` on enums is REQUIRED only for `enumflags2::bitflags` (`#[repr(uN)]`) and external numeric specs (e.g. `u16` for `FontWeight`); decorative `#[repr]` MUST NOT be added. See [`ai-docs/code-style.md` → Enum repr](ai-docs/code-style.md#enum-repr).
...
- **NEVER** batch a `git commit` / data-dependent `AskUserQuestion` in the same turn as the `Edit`/subagent call producing its inputs; verify with `git diff --cached --stat` first. → [§ Dependent tool calls must not be batched](ai-docs/workflow.md#dependent-tool-calls-must-not-be-batched-with-the-call-that-produces-their-inputs)
- **CI-fix commits get self-review too.** Spawn `self-review` before pushing any CI-fix commit. → [§ CI-fix commit self-review (parent rule)](ai-docs/workflow.md#ci-fix-commit-self-review-parent-rule)

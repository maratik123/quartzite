...
pub struct TextEdit {
    ...
    /// Byte offset of the insertion caret within `plain_text`.
    ///
    /// Always in `0..=plain_text.len()`. Managed by [`set_caret`](TextEdit::set_caret).
    pub caret: usize,
    /// Byte offset of the selection anchor within `plain_text`, or `None` when
    /// there is no active selection.
    ///
    /// Always in `0..=plain_text.len()` when `Some`. The selected range is
    /// `min(caret, anchor)..max(caret, anchor)`. Managed by
    /// [`set_selection_anchor`](TextEdit::set_selection_anchor).
...
impl TextEdit {
    ...
    /// Moves the caret to `caret`, clamped to `0..=plain_text.len()`.
    ///
    ...
    /// let mut edit = TextEdit::new();
    /// edit.plain_text = "hello".into();
    /// edit.set_caret(100); // clamps to plain_text.len()
    ...
    pub fn set_caret(&mut self, caret: usize) {
        ...
        let clamped = caret.min(self.plain_text.len());
...
impl TextEdit {
    /// Sets the selection anchor to `anchor`, clamped to `0..=plain_text.len()`.
    ///
    ...
    pub fn set_selection_anchor(&mut self, anchor: Option<usize>) {
        ...
        let clamped = anchor.map(|a| a.min(self.plain_text.len()));
...
mod tests {
    ...
    fn widget_view_returns_text_edit_variant() {
        let edit = TextEdit::new();
        assert_matches!(edit.widget_view(), crate::WidgetView::TextEdit(_));
    }

    // ── Caret + selection field tests (subtask 8) ─────────────────────────
    ...
    #[test]
    fn set_caret_clamps_to_plain_text_len() {
        let mut edit = TextEdit::new();
    ...
    #[test]
    fn set_selection_anchor_clamps_to_plain_text_len() {
        let mut edit = TextEdit::new();
        edit.plain_text = "abc".into();
